...
export const TrainerPortal: React.FC = () => {
    ...
    const loadData = async () => {
        ...
        setRefreshing(true);
        try {
            // Query bookings by Email AND Name to support manual assignments
    ...
    // Load data whenever view changes or trainer is identified
    useEffect(() => {
...
// --- ASSESSMENT WIZARD (Updated for View Only Mode) ---
            ...
            {isLocked && (
                <div className="bg-yellow-50 p-4 rounded-xl border border-yellow-100 mb-6 flex items-start gap-3">
                    ...
                    <div>
                        ...
                        <p className="text-yellow-700 text-xs mt-1">This assessment has been submitted. You can view the details but cannot edit them.</p>
                    </div>
                </div>
            )}

            <div className="flex justify-between items-center mb-6">
                <h2 className="text-xl font-black text-secondary">{isLocked ? 'Client Assessment' : 'New Assessment'}</h2>
                    ...
                    <div className="animate-in slide-in-from-right duration-300 space-y-4">
                        ...
                        <div className="grid grid-cols-2 gap-4">
                            <div>
                                ...
                                <input disabled={isLocked} type="text" className="input" value={data.measurements?.height || ''} onChange={e => updateField('measurements', 'height', e.target.value)} />
                            </div>
                            <div>
                                <label className="label">Weight (kg)</label>
                                <input disabled={isLocked} type="text" className="input" value={data.measurements?.weight || ''} onChange={e => updateField('measurements', 'weight', e.target.value)} />
                            </div>
                        </div>
                    ...
                    <div className="animate-in slide-in-from-right duration-300 space-y-4">
                        <h3 className="text-sm font-bold text-gray-400 uppercase tracking-widest">Fitness Level</h3>
                         ...
                         <div>
                            ...
                            <select disabled={isLocked} className="input" value={data.lifestyle.movement} onChange={e => updateField('lifestyle', 'movement', e.target.value)}>
                            ...
                            <div className="flex gap-2">
                                {['Beginner', 'Intermediate', 'Advanced'].map(lvl => (
                                    <button 
                                        ...
                                        className={`flex-1 py-3 rounded-xl font-bold text-xs border ${
                                            data.fitnessLevel.category === lvl 
                ...
                {step === 3 && (
                    <div className="animate-in slide-in-from-right duration-300 space-y-4">
                         ...
                         <div>
                             ...
                             <input disabled={isLocked} type="text" className="input" value={data.goals.primary} onChange={e => updateField('goals', 'primary', e.target.value)} placeholder="e.g. Weight Loss" />
                         </div>
...
// ... (Other components: SessionHistoryThread, NavButton, etc. remain unchanged) ...
// Re-declaring minor components to keep the file valid
...
const SessionView: React.FC<{ booking: Booking, client: UserProfile, trainerName: string, onClose: () => void }> = ({ booking, client, trainerName, onClose }) => {
    // ... SessionView implementation from previous response, updated with AssessmentWizard usage ...
    // Re-inserting logic to ensure file completeness
    const { showToast } = useToast();
                    ...
                    <div className="bg-white rounded-[32px] p-6 shadow-soft space-y-6">
                        ...
                        {isSessionClosed ? (
                            ...
                            <div className="space-y-4">
                                ...
                                <button onClick={handleFinishSession} disabled={isSubmitting} className="w-full bg-red-500 text-white py-4 rounded-xl font-bold shadow-lg shadow-red-500/20 hover:scale-[1.02] transition-transform flex items-center justify-center gap-2 disabled:opacity-70 disabled:grayscale">
